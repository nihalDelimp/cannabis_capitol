...
import { POT_POLITICS } from '../common/constants'

const Potpolitics = () => {

  const [Potpolitics, setPotpolitics] = useState([])
  const Token = process.env.REACT_APP_VIMEO_TOKEN;

  useEffect(async () => {
    const PotpoliticsData = await axios.get(`${VIMEO_URL}/projects/${POT_POLITICS}/items`,
      {
        headers: {
    ...
    if (PotpoliticsData) {
      ...
      setPotpolitics(PotpoliticsData.data.data.splice(0, 4))
    }
  }, [setPotpolitics]);
  return (
    <>
      {Potpolitics && Potpolitics.length > 0 ?
        (< div className="intertainWraper" >
          <div className="intertainWraperIn">
            <div className="row">
              <div className="col-lg-12">
                <div className="newslistTop" >
                  <h2 className="newslistTop--title sec_title">Pot Politics</h2>
                  <Link className="newslistTop--readmore" to={`/videos/${POT_POLITICS}`}>See More Videos &gt;&gt;</Link>
                </div>
                <div className="news_listBottom">
                  <ul className="newsList">
                    {Potpolitics && Potpolitics?.map((item, index) => (
